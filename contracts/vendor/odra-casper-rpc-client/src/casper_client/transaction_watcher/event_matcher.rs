...
impl EventMatcher {
    ...
    pub fn matches_transaction_hash(
        ...
        // Be resilient: the SSE stream carries many event shapes (FinalitySignature,
        // BlockAdded, keep-alives, etc.). An event we can't parse is simply not the
        // one we're waiting for, so skip it instead of aborting the whole deploy.
